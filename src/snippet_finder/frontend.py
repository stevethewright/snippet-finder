...
class SnippetFinder:
    def __init__(self, root: tkinter.Tk):
        ...
        self.selected_model_label = tkinter.Label(
            self.whisper_opt_frame, text="Selected model:"
        )
        self.selected_model_label.grid(row=0, column=0)
        self.model_combobox = ttk.Combobox(
            self.whisper_opt_frame, values=self.available_models
        )
        self.model_combobox.set("base")
        self.model_combobox.grid(row=0, column=1)
        self.selected_device_label = tkinter.Label(
            self.whisper_opt_frame, text="Selected device:"
        )
        ...
        self.selected_compute_type_label = tkinter.Label(
            self.whisper_opt_frame, text="Selected Compute Type:"
        )
        self.selected_compute_type_label.grid(row=2, column=0)
        self.compute_type_combobox = ttk.Combobox(
            self.whisper_opt_frame, values=self.compute_types
        )
        ...
        self.chosen_file_label = tkinter.Label(
            self.file_select_frame, text="Selected audio file:"
        )
        self.chosen_file_label.grid(row=0, column=0)
        self.selected_file_label = tkinter.Label(
            self.file_select_frame, text="No file selected."
        )
        self.selected_file_label.grid(row=0, column=1)
        self.file_select_button = tkinter.Button(
            self.file_select_frame, text="Select File", command=self.select_file
        )
        ...
        self.fetch_snippets_button = tkinter.Button(
            self.fetch_snippets_frame,
            text="Fetch Snippets",
            command=self.start_transcribe_and_analyse,
        )
